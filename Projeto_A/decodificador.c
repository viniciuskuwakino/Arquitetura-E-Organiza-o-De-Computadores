...
unsigned getImmediate(unsigned int ir) {
	PRINT_FUNC_NAME;
	...
	switch (getOpCode(ir)) {
		case 0: { // 000000, Aritmética.
			...
			switch (getFunct(ir)) {
				...
				case 8:	{ // 000010 -> j address (26), R-Type
					fprintf(stdout, "jr ");
					fprintf(stdout, "%s\n", registerName[getRs(ir)]);
					break;
				}

				case 11:	{ // 001011 (sltu), R-Type
					fprintf(stdout, "sltu ");
					fprintf(stdout, "%s, ", registerName[getRd(ir)]);
		...
		case 35: { // 100011 (lw), I-Type. lw rt, imm(rs)
			fprintf(stdout, "lw ");
			fprintf(stdout, "%s, ", registerName[getRt(ir)]);
			fprintf(stdout, "%#0.8x", getImmediate(ir));
			fprintf(stdout, "(%s)\n", registerName[getRs(ir)]);
			break;
		}

		case 36: { // 100100 (lbu), I-Type
			fprintf(stdout, "lbu ");
			fprintf(stdout, "%s, ", registerName[getRt(ir)]);
			fprintf(stdout, "%#0.8x", getImmediate(ir));
			fprintf(stdout, "(%s)\n", registerName[getRs(ir)]);
